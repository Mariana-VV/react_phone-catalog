...
import p from './ProductCard.module.scss';
import { Link } from 'react-router-dom';
import { CartContext } from '../../../CartModule/store/CartContext';
...
export const ProductCard: React.FC<Props> = ({ product }) => {
  ...
  return (
    <article className={p.prod} key={product.id}>
      <div className={p.image}>
        <img src={product.image} alt={product.name} />
      </div>

      <div className={p.content}>
        <Link to={`/products/${product.id}`} className={p.title}>
          {product.name} {product.year} ID{product.id}
        </Link>
        <div className={p.price}>
          <p className={p.price__new}>${product.price}</p>
          <p className={p.price__full}>${product.fullPrice}</p>
        </div>
        <hr />
        <div className={p.properties}>
          <div className={p.box}>
            <p className={p.key}>Screen</p>
            <p className={p.value}>{product.screen}</p>
          </div>
          <div className={p.box}>
            <p className={p.key}>Capacity</p>
            <p className={p.value}>{product.capacity}</p>
          </div>
          <div className={p.box}>
            <p className={p.key}>RAM</p>
            <p className={p.value}>{product.ram}</p>
          </div>
        </div>
        ...
        <div className={p.buttons}>
          <button
            ...
            className={p.cart__btn}
          >
            Add to cart
          </button>
          <button className={p.favorite__btn}>
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
